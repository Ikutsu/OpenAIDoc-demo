...
---
在任何定义中，你都可以使用注入参数：这些参数将被你的定义注入和使用。
...
以下是注入参数的示例。我们确定需要一个 `view` 参数来构建 `Presenter` 类。我们使用 `params` 函数参数来帮助检索我们注入的参数：
...
如果你有多个相同类型的参数，你可以使用索引 `get(index)`（也与 `[ ]` 运算符相同）而不是使用 `get()` 来解析参数：
...
Koin 图解析（所有定义解析的主树）也允许你查找注入的参数。只需使用常用的 `get()` 函数：
...
- `parameterArrayOf`：使用值的数组，数据将按其索引使用
...
- `parameterSetOf`：使用不同种类的值的集合。不使用索引来滚动值。
...
:::note
你可以使用 `parametersOf` 或 `parameterArrayOf` "级联" 参数注入，以基于索引使用值。或者使用 `parametersOf` 或 `parameterSetOf` 基于类型进行级联以进行解析。
:::
