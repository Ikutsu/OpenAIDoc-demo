...
:::note
请参阅 `libs.versions.toml` 获取当前版本
:::

## 应用概览

应用程序的想法是管理一个用户列表，并在我们的 `MainActivity` 类中使用 Presenter 或 ViewModel 显示它：
...
我们创建一个“Repository”组件来管理用户列表（添加用户或按名称查找一个用户）。 下面是 `UserRepository` 接口及其实现：
...
让我们简单地在 `UserRepositoryImpl` 类上添加 `@Single`，将其声明为单例（singleton）：
...
Koin 模块是从带有 `.module` 扩展名的 `AppModule` 生成的：只需使用 `AppModule().module` 表达式从注解中获取 Koin 模块。
...
`UserViewModel` 使用 `@KoinViewModel` 注解标记，以声明 Koin ViewModel 定义，以便不在内存中保留任何实例（避免 Android 生命周期中的任何泄漏）。
...
通过使用简单的 JUnit 测试验证我们的 Koin 配置，我们可以确保我们的 Koin 配置在启动我们的应用程序之前是良好的。
...
```groovy
// 如果需要，将 Maven Central 添加到您的存储库中
repositories {
	mavenCentral()    
}
