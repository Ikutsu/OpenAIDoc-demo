...
:::info
[源代码可以在 Github 上找到](https://github.com/InsertKoinIO/koin-getting-started/tree/main/android)
:::
...
`UserViewModel` 组件将被创建，同时解析 `UserRepository` 实例。 要在我们的 Activity 中获取它，让我们用 `by viewModel()` 委托函数注入它：
...
我们可以通过使用一个简单的 JUnit 测试验证我们的 Koin 配置来确保我们的 Koin 配置良好，然后再启动我们的应用程序。
