...
`UserPresenter` 组件将被创建，并解析 `UserRepository` 实例。为了在我们的 Activity 中获取它，让我们使用 `by inject()` 委托函数来注入它：
...
:::info
`by inject()` 函数允许我们在 Android 组件运行时（Activity，fragment，Service ...）中检索 Koin 实例
:::
...
我们可以通过使用一个简单的 JUnit 测试来验证我们的 Koin 配置，从而确保我们的 Koin 配置良好，然后再启动我们的应用程序。
